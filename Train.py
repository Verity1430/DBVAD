...
print("Train folder content (videos):", os.listdir(train_folder))
...
print("Number of training samples loaded:", len(train_dataset))
...
# === 安全的 FLOPs 统计（model.cuda() 之后、训练开始之前）===
...
# 1) 备份 RNG 状态（CPU/CUDA）
cpu_state  = torch.get_rng_state()
cuda_state = torch.cuda.get_rng_state()

with torch.no_grad():                          # 禁用 autograd
    _sample = next(iter(train_batch))
    imgs = _sample[0] if isinstance(_sample, (list, tuple)) else _sample
    imgs = imgs.cuda(non_blocking=True)
    inp = imgs[:, :12] if args.method == 'pred' else imgs

    # 随机 memory 仅用于统计 —— 若你担心 RNG，被上面两行状态备份保护
    m_tmp = F.normalize(torch.rand((args.msize, args.mdim), device='cuda'), dim=1)

    # 2) 用“模型副本”统计，且“train=False”禁用你前向里的训练分支与副作用
    model_for_flops = deepcopy(model).cuda()
    flops = FlopCountAnalysis(model_for_flops, (inp, m_tmp, False))
    print(f"[FLOPs] forward-only: {flops.total():.3e}")
    try:
