...
import argparse
import timm
import model.localizer_cnn as localizer_module
...
parser.add_argument('--alpha_img', type=float, default=0.5, help='weight for image-local anomaly score in late fusion')
# ---- 只保存高分帧（新增） ----
parser.add_argument('--save_high_frames', action='store_true', help='save overlay images for high-score frames')
parser.add_argument('--save_threshold', type=float, default=0.7, help='threshold in [0,1] to save frames')
...
parser.add_argument('--temporal_ema', type=float, default=0.7,
                    help='对最终分数做指数滑动平均，范围[0,1)，0 表示关闭')

args = parser.parse_args()

# Verify that the evaluator is importing the matching LocalKNN implementation.
# This avoids a late AttributeError when an older model/localizer_cnn.py is still present.
if not hasattr(LocalKNN, "distance_map") or not hasattr(LocalKNN, "normalize_map"):
    raise RuntimeError(
        "The loaded model/localizer_cnn.py is an older version and does not provide "
        "LocalKNN.distance_map()/normalize_map(). Loaded from: "
        f"{getattr(localizer_module, '__file__', '<unknown>')}. "
        "Replace MNADyuan/model/localizer_cnn.py with the matching file supplied with this evaluator."
    )

# 当前评估循环按单样本和 4 帧上下文实现；显式阻止静默错位。
assert args.test_batch_size == 1, "Current evaluator requires --test_batch_size 1."
assert args.t_length == 5, "Current evaluator is implemented for --t_length 5."
assert 0.0 <= args.alpha_img <= 1.0, "--alpha_img must be in [0,1]."
assert 0.0 < args.image_pooling_p <= 1.0, "--image_pooling_p must be in (0,1]."
assert 0.0 <= args.image_pooling_blend_max <= 1.0, "--image_pooling_blend_max must be in [0,1]."
...
# === Localizer init ===
if args.enable_localizer:
    assert args.localizer_bank is not None, "--enable_localizer requires --localizer_bank."
    assert os.path.isfile(args.localizer_bank),         f"Localizer bank not found: {args.localizer_bank}"

use_local = bool(args.enable_localizer)
if use_local:
...
save_high = use_local and args.save_high_frames and int(args.max_save_per_video) > 0
if save_high:
...
for k, (imgs) in enumerate(test_batch):
    ...
    # === Localizer: 原始距离用于图像级评分；逐帧归一化图仅用于可视化 ===
    if use_local:
        if args.method == 'pred':
        ...
        with torch.no_grad():
            ...
            distance_pyramid = []   # 保留跨帧绝对尺度，用于图像级异常评分
            amap_pyramid = []       # 每帧归一化，仅用于定位/可视化
            ...
            for s in scales:
                ...
                distance_map = knn.distance_map(fmap)
                amap = knn.normalize_map(distance_map)

                distance_up = F.interpolate(
                    distance_map.unsqueeze(1), size=(args.h, args.w),
                    mode='bilinear', align_corners=False
                ).squeeze(1)
                amap_up = F.interpolate(
                    amap.unsqueeze(1), size=(args.h, args.w),
                    mode='bilinear', align_corners=False
                ).squeeze(1)

                # 可选：hflip TTA。原始距离与可视化热图分别融合，避免语义混用。
                if args.local_tta_hflip:
                    cur_s_flip = torch.flip(cur_s, dims=[-1])
                    fmap_f = extractor(cur_s_flip)
                    if args.local_norm:
                        fmap_f = F.normalize(fmap_f, dim=1)
                    distance_map_f = knn.distance_map(fmap_f)
                    amap_f = knn.normalize_map(distance_map_f)

                    distance_f_up = F.interpolate(
                        distance_map_f.unsqueeze(1), size=(args.h, args.w),
                        mode='bilinear', align_corners=False
                    ).squeeze(1).flip(dims=[-1])
                    amap_f_up = F.interpolate(
                        amap_f.unsqueeze(1), size=(args.h, args.w),
                        mode='bilinear', align_corners=False
                    ).squeeze(1).flip(dims=[-1])

                    distance_up = torch.maximum(distance_up, distance_f_up)
                    amap_up = torch.maximum(amap_up, amap_f_up)

                distance_pyramid.append(distance_up)
                amap_pyramid.append(amap_up)

            # 保留原有多尺度逐像素 max 逻辑，但评分使用原始距离。
            distance_up_fused = torch.amax(torch.stack(distance_pyramid, dim=0), dim=0)
            amap_up_fused = torch.amax(torch.stack(amap_pyramid, dim=0), dim=0)

            def pool_image_map(score_map):
                B = score_map.shape[0]
                flat = score_map.view(B, -1)

                def topk_mean(x, frac):
                    k_top = max(1, min(x.size(1), int(x.size(1) * frac)))
                    return torch.topk(x, k_top, dim=1).values.mean(dim=1)

                if args.image_pooling == 'mean':
                    return score_map.mean(dim=(1, 2))
                if args.image_pooling == 'max':
                    return flat.max(dim=1).values
                if args.image_pooling == 'topk':
                    return topk_mean(flat, args.image_pooling_p)
                if args.image_pooling == 'topk+max':
                    tk = topk_mean(flat, args.image_pooling_p)
                    mx = flat.max(dim=1).values
                    w = float(args.image_pooling_blend_max)
                    return (1.0 - w) * tk + w * mx
                if args.image_pooling == 'quantile':
                    return torch.quantile(
                        flat, q=min(max(1.0 - args.image_pooling_p, 0.0), 1.0), dim=1
                    )
                if args.image_pooling == 'lse':
                    N = flat.size(1)
                    return torch.logsumexp(flat, dim=1) - math.log(N)
                return score_map.mean(dim=(1, 2))

            # 正式图像级异常分数来自原始 kNN 距离图。
            img_score = pool_image_map(distance_up_fused)
            # 保存热图仍沿用原脚本的 [0,1] 可视化分数，不改变 save_threshold 语义。
            img_score_for_save = pool_image_map(amap_up_fused)

            # 当前循环只允许 test_batch_size=1。
            img_score_val = float(img_score[0].item())
            img_score_for_save_val = float(img_score_for_save[0].item())
    else:
        img_score_val = 0.0
    ...
    if use_local and save_high:
        ...
        if img_score_for_save_val >= args.save_threshold:
            buf = topk_buffers[cur_video_name]
            item = (float(img_score_for_save_val), int(k), overlay)  # 保存逻辑保持原来的可视化分数
...
# 标签规范：0=正常，1=异常。
labels_anomaly = np.asarray(labels_list, dtype=np.int64).reshape(-1)

# 原MNAD两路融合结果是“越大越正常”：
#   - 归一化PSNR：越大越正常
#   - 反向归一化feature distance：越大越正常
global_normal_score_total_list = []
for video in sorted(videos_list):
    video_name = video.split('/')[-1]
    base_normal_scores = score_sum(
        anomaly_score_list(psnr_list[video_name]),
        anomaly_score_list_inv(feature_distance_list[video_name]),
        args.alpha
    )
    global_normal_score_total_list += base_normal_scores

global_normal_score_total_list = np.asarray(
    global_normal_score_total_list, dtype=np.float32
)

# 统一转换为“越大越异常”。
global_anomaly_score_total_list = 1.0 - global_normal_score_total_list
anomaly_score_total_list = global_anomaly_score_total_list.copy()

# 第三路：原始 kNN 距离池化得到局部异常分数，越大越异常。
if use_local:
    image_score_total_list = []
    if args.per_video_norm:
        # 每个视频内统一做一次正斜率 min-max；不会改变该视频内部AUC排序。
        for video in sorted(videos_list):
            video_name = video.split('/')[-1]
        ...
        image_score_total_list = np.asarray(
            image_score_total_list, dtype=np.float32
        )
    else:
        for video in sorted(videos_list):
            video_name = video.split('/')[-1]
        ...
        if image_score_total_list.size > 0:
            img_min = float(image_score_total_list.min())
            img_max = float(image_score_total_list.max())
            if img_max > img_min:
                image_score_total_list = (
                    image_score_total_list - img_min
                ) / (img_max - img_min)
            else:
                image_score_total_list = np.zeros_like(
                    image_score_total_list, dtype=np.float32
                )

    # 两路现已统一为“越大越异常”，直接同向融合。
    anomaly_score_total_list = (
        (1.0 - args.alpha_img) * global_anomaly_score_total_list
...
if use_ema:
    ema = None
    out = []
    for score in anomaly_score_total_list.tolist():
        ema = (
            score
            if ema is None
            else ema_gamma * ema + (1.0 - ema_gamma) * score
        )
        out.append(ema)
    anomaly_score_total_list = np.asarray(out, dtype=np.float32)

# 强制检查标签、长度和数值，避免静默输出错误指标。
assert labels_anomaly.size == anomaly_score_total_list.size, (
    f"Label/score length mismatch: labels={labels_anomaly.size}, "
    f"scores={anomaly_score_total_list.size}"
)
assert set(np.unique(labels_anomaly).tolist()).issubset({0, 1}), (
    f"Labels must use 0=normal, 1=anomaly, got {np.unique(labels_anomaly)}"
)
assert np.unique(labels_anomaly).size == 2, (
    "ROC/AP require both normal and anomaly labels."
)
assert np.all(np.isfinite(anomaly_score_total_list)), (
    "Final anomaly scores contain NaN or Inf."
)

# 正类统一为异常：标签1=异常，分数越大越异常。
y_true = labels_anomaly
y_score = np.asarray(anomaly_score_total_list, dtype=np.float64)

accuracy = roc_auc_score(y_true, y_score)

print('The result of ', args.dataset_type)
print('Positive class: anomaly (label=1), higher score = more anomalous')
...
# Best-F1：precision/recall最后一个点没有对应threshold，因此使用[:-1]。
...
export_path = os.path.join(
    export_root,
    f"roc_{model_tag}_anomaly_positive.npz"
)
...
np.savez(
    ...
    positive_class=np.asarray(['anomaly']),
    label_convention=np.asarray(['0=normal,1=anomaly']),
...
plt.title('ROC Curve (Anomaly as Positive Class)')
plt.legend(loc="lower right")
roc_path = os.path.join(
    roc_root,
    f'ROC_{args.dataset_type}_anomaly_positive.png'
)
...
# ===== 这里改成可以自己调帧数区间 =====
FRAME_START = 10100    # 起始帧下标（含）
FRAME_END = 10800    # 结束帧下标（不含），设为 None 表示一直到最后一帧
# =================================
...
for video in sorted(videos_list):
    ...
    # 当前 anomaly_score_total_list 已经是“越大越异常”。
    # 直接绘制最终异常分数，不再取反，也不再对子区间二次归一化。
    y = scores

    # 使用实际裁剪后的区间，避免 FRAME_END 超出视频长度时横纵轴长度不一致。
    ...
    for st, ed in _ranges_from_labels(labels_v, anomaly_value=1):
        ax.axvspan(start + st, start + ed, color='skyblue', alpha=0.3)
